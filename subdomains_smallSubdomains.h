...
void constructTerritoriesHost(meshPartitionForStage &partition, linearSystem matrix, uint32_t * iterationLevel, uint32_t N, uint32_t nSub, uint32_t stageID)
{
	...
	if (stageID == 0) numExpansionSteps = (nPerSub-2); // nPerSub-2; // N/3-2; // Originally N/2-2
	if (stageID == 1) numExpansionSteps = (nPerSub-2); // N/3-2; // Originally N/2-2
	if (stageID == 2) numExpansionSteps = 2*(nPerSub-2); // 2*(nPerSub-2); // N-4; // N-6; // originally N-4 // N/2-1;
	if (stageID == 3) numExpansionSteps = determineMaxLevel(iterationLevel, N*N); // 2*(nPerSub-2); // 2*(nPerSub-2)+1; // N-6; // originally N-3; // N/2; // N/2;

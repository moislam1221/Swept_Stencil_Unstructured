...
int main (int argc, char * argv[]) 
{
	...
	stageAdvanceJacobiPerformanceV2OverlapExperimental<<<bridge.numSubdomains, 512, bridge.sharedMemorySize>>>(evenSolutionBufferOutput_d, oddSolutionBufferOutput_d, evenSolutionBuffer_d, oddSolutionBuffer_d,  iterationLevel_d, iterationLevelOutput_d, bridge_d, 0, numJacobiIters, Ndofs);
	...
	// V1 kernel (non-overlapping)
	stageAdvanceJacobiPerformance<<<lowerPyramidal.numSubdomains, 512, lowerPyramidal.sharedMemorySize>>>(evenSolutionBuffer_d, oddSolutionBuffer_d, iterationLevel_d, lowerPyramidal_d, 0, numJacobiIters); 
	// V2 kernel (overlapping)
	...
	stageAdvanceJacobiPerformanceV2OverlapExperimental<<<dualBridge.numSubdomains, 512, dualBridge.sharedMemorySize>>>(evenSolutionBufferOutput_d, oddSolutionBufferOutput_d, evenSolutionBuffer_d, oddSolutionBuffer_d,  iterationLevel_d, iterationLevelOutput_d, dualBridge_d, 0, numJacobiIters, Ndofs);
	tmp = evenSolutionBuffer_d; evenSolutionBuffer_d = evenSolutionBufferOutput_d; evenSolutionBufferOutput_d = tmp;
	tmp = oddSolutionBuffer_d; oddSolutionBuffer_d = oddSolutionBufferOutput_d; oddSolutionBufferOutput_d = tmp;
	tmpInt = iterationLevel_d; iterationLevel_d = iterationLevelOutput_d; iterationLevelOutput_d = tmpInt;

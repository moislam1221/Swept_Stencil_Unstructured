...
int main (int argc, char * argv[]) 
{
	...
	linearSystem matrix;
	matrix.Ndofs = Ndofs;
	initializeAndLoadMatrixFromDirectory(matrix, "Square_Mesh");   
	...
	uint32_t numIterations = 20; // 2 * (nPerSub-1) * numSweptCycles;
...
*/	
	...
	iterationLevelFile.open("Square_Mesh/iteration_1.txt");
	territoriesFile.open("Square_Mesh/territories_1.txt");
	...
	bridge.numSubdomains = 1;
	set<int> seed2_1;
	int bridgeSeed;
	int numSeeds = 222;
	std::ifstream bridgeSeedFile1("Square_Mesh/bridgeSeed.txt");	
...
*/	
	// Spit out iteration level for visualization in Python
  	// ofstream iterationLevelFile;
  	// ofstream territoriesFile;
	iterationLevelFile.open("Square_Mesh/iteration_2.txt");
	territoriesFile.open("Square_Mesh/territories_2.txt");
	distanceFile.open("Square_Mesh/distanceFromSeed_2.txt");
	for (int i = 0; i < matrix.Ndofs; i++) {
		// printf("Iteration Level[%d] = %d\n", i, iterationLevel[i]);
  		iterationLevelFile << iterationLevel[i];
  		iterationLevelFile << "\n";
		...
		if (bridge.subdomainOfDOFs[i] == UINT32_MAX) {
  			...
  			distanceFile << bridge.distanceFromSeed[i];
  		}
		distanceFile << "\n";
	...
	set<int> seed3_1 = {116, 134, 144, 149, 165, 173};
	set<int> seed3_2 = {200, 222, 236};
	...
	numJacobiSteps = 10; // numJacobiStepsIncrement-1;
...
*/
	...
	// Spit out iteration level for visualization in Python
  	// ofstream iterationLevelFile;
  	// ofstream territoriesFile;
	iterationLevelFile.open("Square_Mesh/iteration_3.txt");
	territoriesFile.open("Square_Mesh/territories_3.txt");
	for (int i = 0; i < matrix.Ndofs; i++) {
		// printf("Iteration Level[%d] = %d\n", i, iterationLevel[i]);
  		iterationLevelFile << iterationLevel[i];
  		iterationLevelFile << "\n";
	...
	int dualBridgeSeed;
	numSeeds = 172;
	std::ifstream dualBridgeSeedFile1("Square_Mesh/dualBridgeSeed.txt");	
	for (int i = 0; i < numSeeds; i++) {
...
*/
	// Spit out iteration level for visualization in Python
  	// ofstream iterationLevelFile;
  	// ofstream territoriesFile;
	iterationLevelFile.open("Square_Mesh/iteration_4.txt");
	territoriesFile.open("Square_Mesh/territories_4.txt");
	for (int i = 0; i < matrix.Ndofs; i++) {
		// printf("Iteration Level[%d] = %d\n", i, iterationLevel[i]);
  		iterationLevelFile << iterationLevel[i];
  		iterationLevelFile << "\n";
	...
	stageAdvanceJacobiPerformance<<<upperPyramidal.numSubdomains, 512, upperPyramidal.sharedMemorySize>>>(topSweptSolution_d, bottomSweptSolution_d, iterationLevel_d, upperPyramidal_d, 0, numJacobiSteps, numJacobiSteps+5, finalStage);
	// printDeviceArrayInt(iterationLevel_d, Ndofs);
	// printf("================================================\n");
	// printDeviceSimilarity1D(topSweptSolution_d, solutionGM, 1e-6, Ndofs);
	// cudaDeviceSynchronize();
	...
	// Spit out iteration level for visualization in Python
	iterationLevelFile.open("Square_Mesh/iteration_final.txt");
	for (int i = 0; i < matrix.Ndofs; i++) {
		// printf("Iteration Level[%d] = %d\n", i, iterationLevel[i]);
  		iterationLevelFile << iterationLevel[i];
  		iterationLevelFile << "\n";

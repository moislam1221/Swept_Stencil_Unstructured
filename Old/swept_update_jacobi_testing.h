void determineSharedMemoryAllocation(meshPartitionForStage &partition, bool lowerPyramidal = false)
{
	...
	for (int i = 0; i < partition.numSubdomains; i++) {
		...
		numDOFsInteriorPlus1 = partition.territoryIndexPtrInterior[i+1] - partition.territoryIndexPtrInterior[i]+1;
...
void determineSharedMemoryAllocationNew(meshPartitionForStageNew &partition, bool lowerPyramidal = false)
{
	// Determine the appropriate shared memory allocation (sum of twice the subdomain DOFs plus number of nnz entries for offdiags)
	uint32_t sharedMemorySize = 0;
	uint32_t sharedMemorySizeTemp;
	...
	for (int i = 0; i < partition.numSubdomains; i++) {
		numDOFs = partition.territoryIndexPtr[i+1] - partition.territoryIndexPtr[i];
		nnz = partition.indexPtrDataShiftSubdomain[i+1] - partition.indexPtrDataShiftSubdomain[i];
		numDOFsInteriorPlus1 = partition.territoryIndexPtrInteriorExt[i+1]-partition.territoryIndexPtrInteriorExt[i]+1;
		if (lowerPyramidal == false) {
			...
			sharedMemorySizeTemp = (2 * numDOFs) * sizeof(float) + numDOFs * sizeof(int) + (2 * numDOFs) * sizeof(float) + nnz * sizeof(float); 
		}
		if (sharedMemorySize < sharedMemorySizeTemp) {
			sharedMemorySize = sharedMemorySizeTemp;
		}
...
__global__
void stageAdvanceJacobiPerformanceNew(float * evenSolutionBuffer, float * oddSolutionBuffer, uint32_t * iterationLevels, meshPartitionForStageDeviceNew partition, uint32_t numJacobiStepsMin, uint32_t numJacobiStepsMax, uint32_t maxIterShift = 0, bool finalStage = false)
{
	...
	uint32_t numDOFsToUpdate = partition.territoryIndexPtrInteriorExt_d[blockIdx.x+1] - partition.territoryIndexPtrInteriorExt_d[blockIdx.x];
	// __syncthreads();
	...
	uint32_t lowerIdx = partition.territoryIndexPtrInteriorExt_d[blockIdx.x];
	uint32_t dist;
	uint32_t subdomain;
	uint32_t n, ni;
	...
	if (i < numDOFsToUpdate) {
		g = partition.territoryDOFs_d[idx_lower + i];
		myGroundLevel = iterationLevelShared[i]; ;
		if (finalStage == 0) {
			for (int iter = numJacobiStepsMin; iter < numJacobiStepsMax; iter++) {
				// printf("iter = %d\n", iter);
				if (iter >= iterationLevelShared[i] && iter < partition.maximumIterations_d[lowerIdx + i] + maxIterShift) {
...
__global__
void assembleSolutionFromBuffers(float * solution_d, float * evenSolutionBuffer_d, float * oddSolutionBuffer_d, uint32_t * iterationLevel_d) 
{
...
__global__
void stageAdvanceJacobiPerformance(float * topSweptSolution, float * bottomSweptSolution, uint32_t * iterationLevels, meshPartitionForStageDevice partition, uint32_t numJacobiStepsMin, uint32_t numJacobiSteps, uint32_t numThresholdIters, bool finalStage = false)
{
	extern __shared__ float sharedMemorySolution[];
		...
		for (int iter = numJacobiStepsMin; iter < numJacobiSteps; iter++) {
			if ((int)partition.distanceFromSeed_d[g] + iter < numThresholdIters && myGroundLevel <= iter) {
				...
				for (int k = sharedIndexPtr[i]; k < sharedIndexPtr[i+1]; k++) { // shared
					...
					du1[i] -= sharedOffDiags[k] * du0[n]; // shared
					/* if (g == 3 || g == 24) {
						printf("Block %d: Iter = %d: g = %d - Decrease value by %f = %f * %f\n", blockIdx.x, iter, g, sharedOffDiags[k] * du0[n], sharedOffDiags[k], du0[n]);
					} */
...
*/
	...
	if (i < partition.interiorDOFsPerSubdomain_d[blockIdx.x]) {
		...
		//	if (dist % 2 == 0) {
		if (myGroundLevel % 2 == numJacobiSteps % 2) {
